 ...
 * @param {object} param.routes 路由配置 {constantRoutes, asyncRoutes}
 * @param {object} param.handlers http请求结合 { getMenus }
 * @returns 
 */
export default function createPermission({ routes = {}, getMenus, resetRouter } = {}) {
  const { constantRoutes = [], asyncRoutes = [] } = routes

  let notFoundRoute = null
  const lastRoute = constantRoutes[constantRoutes.length - 1]
  if (lastRoute.path === '*' || lastRoute.name === 'NotFound') {
    notFoundRoute = lastRoute
    constantRoutes.splice(-1)
  }
  ...
  const actions = {
    ...
    resetRouter: {
      // 挂载根节点。调用：dispatch('resetRouter', null, { root: true })
      root: true,
      handler() {
